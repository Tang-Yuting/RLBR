...
class BaggedRewardsTransformer(object):
    ...
    def _train_pref_step(self, train_states, rng, batch):
        def loss_fn(train_params, rng):
            ...
            bagged_reward_target = jax.lax.stop_gradient(bag_end)
            bagged_reward_ave = jax.lax.stop_gradient(ave_reward)
            trans_loss = bagged_reward_loss(bagged_reward_pred, bagged_reward_ave, bagged_reward_target)
            cse_loss = trans_loss
