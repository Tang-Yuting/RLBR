...
class TransRewardModel(nn.Module):
    ...
    def __call__(
        ...
        hidden_output = x[:, reward_idx]
        hidden_state_output = x[:, state_idx]
        state_output = nn.Dense(features=self.observation_dim)(hidden_state_output)
        ...
        if self.config_.use_weighted_sum:
            ...
            return {"value": output, "state_pred": state_output}, attn_weights_list
