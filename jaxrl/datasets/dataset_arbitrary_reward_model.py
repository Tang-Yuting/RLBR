...
class Dataset_Arbitrary_Reward_Model(object):

    def __init__(self, observations: np.ndarray, actions: np.ndarray,
                 real_rewards: np.ndarray, masks: np.ndarray,
                 dones_float: np.ndarray, next_observations: np.ndarray,
                 rewards_pred: np.ndarray, bag_end: np.ndarray, bag_label: np.ndarray, size: int):
    ...
    def sample_sequences(self, num_query, len_query, sampled_indices):
        ...
        total_bag_label_seq = np.zeros((num_query, len_query), dtype=np.int32)
        ...
        for query_count, i in enumerate(tqdm(query_range, desc="get queries from saved indices")):
            ...
            bag_label_seq = self.bag_label[start_idx:end_idx]
            ...
            total_bag_label_seq[query_count] = bag_label_seq
        ...
        seq_bag_label = total_bag_label_seq.copy()
    ...
    def recompute_rewards(self,
        ...
        new_r = np.zeros_like(self.real_rewards)
        pts = []
        attn_weights = []
