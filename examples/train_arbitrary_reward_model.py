...
from jaxrl.agents import (AWACLearner, DDPGLearner, REDQLearner, SACLearner,
                          SACV1Learner)
from jaxrl.datasets import ReplayBuffer_Arbitrary_Reward_Model
from jaxrl.evaluation import evaluate
from jaxrl.utils import make_env

from collections import defaultdict
import transformers
from BaggedRewardModel.Transformer_arb import BaggedRewardsTransformer
...
def main(_):
    ...
    replay_buffer = ReplayBuffer_Arbitrary_Reward_Model(env.observation_space, env.action_space,
    ...
    for i in tqdm.tqdm(range(1, FLAGS.max_steps + 1),
        ...
        if done or (i == FLAGS.max_steps):
            bag_begin = np.zeros(len(trajectory))
            bag_end = np.zeros(len(trajectory))
            t = 0
            bag_begin[0] = 1
            arbitrary_bag_len = random.randint(100, 500)

            while (t + arbitrary_bag_len) < len(trajectory):
                t = t + arbitrary_bag_len
                bag_end[t] = 1
                repeat = 0
                if bag_end[-1] == 1:
                    break
                if (t+repeat) < len(trajectory):
                    t = t+repeat
                    bag_begin[t] = 1
                    arbitrary_bag_len = random.randint(100, 500)
                else:
            ...
            for j in range(len(trajectory)):
                trajectory[j]['bag_label'] = sum(bag_begin[:j+1]) - sum(bag_end[:j])
                trajectory[j]['bag_end'] = bag_end[j]
