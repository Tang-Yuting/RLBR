...
def main(_):
    ...
    bag = []
    traj_reward = []
    arbitrary_bag_len = random.randint(25, 200)

    min_reward, max_reward = 50000, -50000
    # repeat = random.randint(-20, 20)
    ...
    for i in tqdm.tqdm(range(1, FLAGS.max_steps + 1),
        ...
        if (not done) and (len(bag) == arbitrary_bag_len):
            # print("add bag", i)
            # print("reward_1", [t['rews'] for t in bag])
            repeat = random.randint(-20, 20)
            real_bag_len = arbitrary_bag_len + repeat

            # (done) TODO: add ave or sum bagged reward information
            if FLAGS.bag_type == 'ave':
                # sum
                # mean_reward = sum([t['rews'] for t in bag]) / len(bag)
                mean_reward = sum(traj_reward[-real_bag_len:]) / real_bag_len

                for trans in bag[-real_bag_len:]:
                ...
                total_reward = sum(traj_reward[-real_bag_len:])
        ...
        if done or (i == FLAGS.max_steps):
            repeat = random.randint(-20, 20)
            real_bag_len = max(1, len(bag) + repeat)
